...
//For a given value of an info field in the list, ensure that the number of repetitions (duplicates) of that number is even. 
//If for the given value of the info field, the number of elements with that value is odd, an element with the same value should be added before the first appearance of the value.
...
public class DeleteSLL {
    ...
    public static void change(SLL<Integer> list, int br) {
        SLLNode<Integer> number = list.getFirst();
        int even = 0;

        while(number != null){
            if(number.element.equals(br)){
                even++;
            }
            number = number.succ;
        }
