...
public class ListaOdListi {
    ...
    public static long findMagicNumber(DLL<DLL<Integer>> list) {
        ...
        DLLNode<DLL<Integer>> elem = list.getFirst(); //the first in the outerDLL
        DLLNode<Integer> l = elem.element.getFirst(); //the first in the inner DLL
        long prod = 1;

        while (elem != null) {
            l = elem.element.getFirst();
            long sum = 0;
            while(l != null){
                sum += l.element;
                l = l.succ;
