...
Write a function that, for a binary search tree and a selected element T from the tree will: find the depth of element T in the tree
...
public class DepthOfNode{
    private static int getDepth(BNode<Integer> node, int x, int depth) {
        if (node == null)
            return -1;
        ...
        if (node.info.compareTo(x) == 0)
            ...
            return getDepth(node.left, x, depth + 1);
        else
            return getDepth(node.right, x, depth + 1);
    }

    public static int getDepth(BinarySearchTree<Integer> tree, int x) {
        return getDepth(tree.getRoot(), x, 0);
    }
        ...
        for(int i = 0; i < n + m; i++){
            ...
            if(line.equals("insert")){
               tree.insert(number);
                ...
                System.out.println(getDepth(tree, number) + 1);
            }
        }
