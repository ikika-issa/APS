...
public class Main{
    public static void DeleteSpecial(SLL<String> list, int k){
        ...
        //this loop is to find the original end of the list, so that it doesn't go in an infinite loop
        while(word.succ != null){
            word = word.succ;
        }
        ...
        SLLNode<String> keep = null;
        SLLNode<String> newsucc = null;

        while(word != stopPoint.succ){
            SLLNode<String> next = word.succ;

            if(word.element.length() == k){
                keep = word;
                newsucc = keep;
                list.delete(keep);
                list.insertLast(newsucc.element);
            }
            word = next;
        }
