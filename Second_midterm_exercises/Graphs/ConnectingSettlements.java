//A newly established city needs to connect its neighborhoods with roads. The mayor wants to ensure that every neighborhood is accessible from any other neighborhood while minimizing the cost of building the roads. 
//A list of possible roads and the cost of building each road is given. Determine the minimum cost to connect all neighborhoods.
//Input: The first line contains the number of neighborhoods N. The next N lines contain the names of the neighborhoods. 
//The next line contains the number of possible roads M, followed by M lines that contain the two neighborhoods connected by each road and the cost of building that road.
...
public class ConnectingSettlements{
    public static void main(String[] args){
        Scanner sc = new Scanner(System.in);
        int N = sc.nextInt();

        Map<String, Integer> settlementIDs = new HashMap<>();
        Map<Integer, String> settlementNames = new HashMap<>();
        AdjacencyMatrixGraph<String> graph = new AdjacencyMatrixGraph<>(N);

        for(int i = 0; i < N; i++){
            String settlement = sc.next();

            settlementNames.put(i, settlement);
            settlementIDs.put(settlement, i);
            graph.addVertex(i, settlement);
        }

        int M = sc.nextInt();

        for(int i = 0; i < M; i++){
            String settlement1 = sc.next();
            String settlement2 = sc.next();

            graph.addEdge(settlementIDs.get(settlement1), settlementIDs.get(settlement2), sc.nextInt());
        }
